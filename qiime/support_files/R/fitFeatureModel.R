...
"fitFeatureModel" <- function(input_path, out_path, mapping_category, subcategory_1, subcategory_2) {
            ...
            if (is.null(observation_metadata(foo))) {
            	MRtable(fit, number = nrow(assayData(MGS)$counts), group=3, file=out_path)
            } else {
				res = MRtable(fit, number = nrow(assayData(MGS)$counts), group=3)
...
fitFeatureModel(opts$input_path, opts$out_path, opts$mapping_category, opts$subcategory_1, opts$subcategory_2)
